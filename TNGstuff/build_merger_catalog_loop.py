...
def main():
    ...
    for j in range(nmerg):
        ...
        if snapnum_full - snapnum > 0: #this would happen for snapnum 49 i.e., you want to get descendants one time step forward
            ...
            try:
                # I BELIEVE THIS SHOULD BE ALL_MASS_F BUT DON'T WANT TO CHANGE IT FROM THE ORIGINAL
...
for snapnum in snapnum_list:
    ...
    if all_prog:
        file_check = 'Tables/mergers_snap_all_prog_'+str(snapnum)+'_'+run+'.txt'
    else:
        file_check = 'Tables/mergers_snap_'+str(snapnum)+'_'+run+'.txt'

...
# This code matches at individual snapshots
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# There are bunch of different versions of matching, all of which are defined
# within the find_match code, which is from Bobby :)
version = 'no_enviro'
#'enviro'
#'no_enviro'
#'no_enviro_tight'
...
# Number of Gyr ago that a matched non-merge must not have merged
years_ago = 2.0
...
def find_match(version, mass, r1, r2, N2, df):
    ...
    while found == False:
        ...
        if version == 'enviro' or version == 'no_time_enviro':
        ...
        if version == 'N_r_1_enviro':
            indices = df.index[(df['num_mergers_in_last_2Gyr']==0.0) &
                              ...
                              (df['N']-N2_tol<=N2) &
                              (N2<=df['N']+N2_tol)].tolist()
        if version == 'r_1_enviro':
            indices = df.index[(df['num_mergers_in_last_2Gyr']==0.0) &
                              (np.log(df['Mass'])-Mstar_tol<=np.log(mass)) &
                              (np.log(mass)<=np.log(df['Mass'])+Mstar_tol) & 
                              (df['r_1']-r1_tol<=r1) &
                              (r1<=df['r_1']+r1_tol)].tolist()
        if version == 'N_enviro':
            indices = df.index[(df['num_mergers_in_last_2Gyr']==0.0) &
                              (np.log(df['Mass'])-Mstar_tol<=np.log(mass)) &
                              (np.log(mass)<=np.log(df['Mass'])+Mstar_tol) &
...
#snapnum_list = [17,25,33,40,50,59,67,72,84,99]
snapnum_list = [40,50,59,67,72,84,99]
#snapnum_list = [84]
#snapnum_list = [33]
#snapnum_list = [50]
for snapnum in snapnum_list:
    ...
    # Load in massive merger tables that has all mergers within 0.5 Gyr of the bin center
    mergers = pd.read_csv('Tables/Aimee_SF/'+str(snapnum)+'/all_mergers_in_bin.txt', sep='\t')
    ...
    # Again, my matching is different than Bobbys in that I'm not allowing the snapnum to change
    for snap in mergers['center'].unique():
        print(mergers['center'].unique())
        print('unique snaps', snap)
        ...
        if years_ago != 0.0:
            ...
            print('snap check', str(snap))
            print('lbt snap', lbt_snap)
            include_these_snaps = []
            ...
            for w in range(10):
                ...
                y = w + 1
                print('y', y)
                print('key', str(snap - y))
                #print('if condition', lbt_dict[str(snap - y)] - lbt_snap)
                try:
                    print('trying')
                    if str(snap - y) in lbt_dict:
                        print('in it')
                        if lbt_dict[str(snap - y)] - lbt_snap > years_ago:
                            print('breaking')
                            break
                        else:
                            print('lbt', lbt_snap, 'snap', snap)
                            print('snap', snap-y)
                            print('lbt', lbt_dict[str(snap-y)])
                    ...
                    #break
                    print('does not exist in the dictionary', 'snap', str(snap - y))
                    continue
            ...
            except: #include_these_snaps might be empty
                n_steps = 1
        else:
            n_steps = 1
        ...
        try:
            ...
            for id in merg_i:
                ...
                fields = ['SubhaloID', 'NextProgenitorID', 'MainLeafProgenitorID',
                          'FirstProgenitorID', 'SubhaloMassType','DescendantID','SubfindID', 'RootDescendantID']
                
                # Need to set onlyMPB = False in order to do merger analysis with the three:
                print('snap check', snap)
                ...
                if tree:
                    ...
                    except IndexError: #I think this means no stars for some reason
                        n_merg_2Gyr = 99
                else:
                    n_merg_2Gyr = 99
        ...
        try:
            ...
            for p in select_indices:
                ...
                try:
                    ...
                    #print('tree', tree)
                    #print(sP)
                    # print('snap for failing tree', snap)
                    # print('id for failing tree', p)
                    # print('RootID', len(tree['RootDescendantID']))
                    # print(tree['RootDescendantID'])
                    n_merg_2Gyr = u.num_mergers_n_step(sP, tree, n_steps, minMR, massPartType='stars', index=1)
                except IndexError:
                    print('failed because of indexes at snap ' + str(snap) + 'subhalo' + str(p))
                    pass
            ...
            #ax1.set_xlim([0,100])
            plt.legend()
            plt.savefig('Figs/Aimee_SF/hist_N_2Mpc_all_subhalos_'+str(snapnum)+'_center_'+str(snap)+'_'+str(version)+'_'+str(years_ago)+'.png')
            ...
            #ax1.set_xlim([0,100])
            plt.legend()
            plt.savefig('Figs/Aimee_SF/hist_r_1_all_subhalos_'+str(snapnum)+'_center_'+str(snap)+'_'+str(version)+'_'+str(years_ago)+'.png')
            ...
            print('length of df_non_merg', len(matches))
            print(matches)
            
            # UGHHHHHHH now sort by the subids list
            # new_df_full.SubhaloID = new_df_full.SubhaloID.astype("category")
            # print('new_df_full length', len(new_df_full))
            # new_df_full.SubhaloID.cat.set_categories(subids) #,inplace=True inplace is depricated
            ...
            #above is depricated -- this is the update from chat GPT
            # Assuming new_df_full is your DataFrame and subids is your list of category values

            # Convert SubhaloID column to category type
            new_df_full["SubhaloID"] = pd.Categorical(new_df_full["SubhaloID"], categories=subids)
            # Sort DataFrame based on SubhaloID
            new_df_full = new_df_full.sort_values(by=["SubhaloID"])
            # Drop rows with NaN values in SubhaloID column
            new_df_full = new_df_full.dropna(subset=["SubhaloID"])
            print('new_df_full length', len(new_df_full))
            
                        
            print('new_df_full length', len(new_df_full))
            #print('match_this UNIQUE length', len(match_this.unique()))

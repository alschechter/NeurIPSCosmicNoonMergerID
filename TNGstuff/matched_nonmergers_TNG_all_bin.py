...
for snapnum in snapnum_list:
    ...
    for snap in mergers['center'].unique():
        ...
        try:
            selection = pd.read_csv('Tables/Aimee_SF/'+str(snapnum)+'/all_mergers_in_bin_enviro.txt', sep = '\t')
            selection = selection[selection['center']==snap]
            ...
            selection_enviro.to_csv('Tables/Aimee_SF/'+str(snapnum)+'/all_mergers_in_bin_enviro.txt', sep = '\t')
        
            
        # Now, try to see if the non-merger table already exists:
        try:
            selection = pd.read_csv('Tables/Aimee_SF/'+str(snapnum)+'/nonmergers_matched_in_bin_'+str(version)+'.txt', sep = '\t')
            ...
            matched_non_list.append(new_df_full)
    all_nonmergers = pd.concat(matched_non_list, axis=0)
    # Reset index
    all_nonmergers = all_nonmergers.reset_index()
    print('almost done')
    print(all_nonmergers)
    all_nonmergers.to_csv('Tables/Aimee_SF/'+str(snapnum)+'/nonmergers_in_bin_'+str(version)+'.txt', sep = '\t')
